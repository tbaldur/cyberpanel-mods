...
install_php_version() {
  if ! (yum -y install lsphp$1* 2> /dev/null || apt-get install -y lsphp$1* 2> /dev/null || dnf install -y lsphp$1* 2> /dev/null); then
    echo "Failed to install PHP $1. Package may not exist or package manager (yum/apt-get/dnf) may not be available."
    exit 1
  fi
}
